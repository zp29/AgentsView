...
| 状态栏主界面 | 轨道 logo + **运行中总数**（0 时隐藏数字）+ 状态胶囊边框；点开面板看列表 |
...
- 状态栏：**聚合运行中数量**（不再默认 `CC:n GPT:m`）；有任务时绿色胶囊边框缓慢旋转，待处理红橙，刚完成红闪后消失。
- 已完成默认范围：**今日**。
- 久未活动：≥45 分钟无 Hook **只列表标记**，不自动改完成；用户可右键手动改状态。
- 技术：**原生 Swift**（MenuBarExtra）。
- 分发：GitHub Release DMG（ad-hoc，未公证）+ 开源自行打包；不做 Sparkle。
## 架构
